            ...
            <div class="grid lg:grid-cols-2 gap-12 items-center">
                ...
                <div class="text-left" x-data x-intersect="$el.classList.add('animate-fade-in')">
                    ...
                    <div class="flex flex-col sm:flex-row gap-4">
                        <a href="{{ route('map.index') }}" class="group inline-flex items-center justify-center px-8 py-4 text-base font-semibold text-white bg-blue-600 rounded-xl hover:bg-blue-700 transition-all shadow-lg hover:shadow-xl hover:-translate-y-0.5">
                        ...
                        @guest
                        <a href="{{ route('register') }}" class="group inline-flex items-center justify-center px-8 py-4 text-base font-semibold text-blue-600 bg-white border-2 border-blue-600 rounded-xl hover:bg-blue-50 transition-all">
                                ...
                                <div class="bg-blue-50 rounded-xl p-6 border border-blue-200">
                                    ...
                                    <div class="grid grid-cols-2 gap-3">
                                        <div class="bg-white rounded-lg p-4 border border-blue-100">
                                            <div class="text-2xl font-bold text-blue-600">{{ \App\Models\UmkmProfile::count() }}</div>
                                            <div class="text-xs text-gray-600 mt-1">Total UMKM</div>
                                        </div>
                                        <div class="bg-white rounded-lg p-4 border border-blue-100">
                                            <div class="text-2xl font-bold text-green-600">{{ \App\Models\UmkmProfile::where('is_verified', true)->count() }}</div>
...
<section class="py-20 bg-gradient-to-b from-gray-50 to-white">
    <div class="container">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            ...
            <div class="group" x-data x-intersect="$el.classList.add('animate-fade-in')" style="animation-delay: {{ $index * 0.1 }}s;">
                <div class="h-full bg-white rounded-2xl p-8 border-2 border-gray-100 hover:border-{{ $pillar['color'] }}-200 hover:shadow-xl transition-all duration-300 group-hover:-translate-y-1">
                    <div class="w-14 h-14 bg-{{ $pillar['color'] }}-100 rounded-xl flex items-center justify-center mb-6 group-hover:scale-110 transition-transform">
                        <svg class="w-7 h-7 text-{{ $pillar['color'] }}-600" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" d="{{ $pillar['icon'] }}"/>
                        </svg>
                    </div>
                    <div class="w-10 h-10 bg-{{ $pillar['color'] }}-600 rounded-full flex items-center justify-center mb-4">
                        <span class="text-white font-bold text-lg">{{ $pillar['number'] }}</span>
                    </div>
...
<section class="py-20 bg-white relative">
    <div class="container">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <div class="group" x-data x-intersect="$el.classList.add('animate-fade-in')" style="animation-delay: {{ $index * 0.1 }}s;">
                <div class="bg-white rounded-2xl shadow-sm hover:shadow-lg transition-all duration-300 p-6 border border-gray-100 hover:border-{{ str_replace('bg-', '', str_replace('-50', '-200', $stat['bgColor'])) }} group-hover:-translate-y-1">
                    <div class="flex items-start justify-between mb-4">
                        <div class="w-14 h-14 {{ $stat['bgColor'] }} rounded-xl flex items-center justify-center group-hover:scale-110 transition-transform">
                            <svg class="w-7 h-7 {{ $stat['iconColor'] }}" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" d="{{ $stat['icon'] }}"/>
                            </svg>
                        </div>
                    </div>
                    <h3 class="text-3xl font-bold {{ $stat['valueColor'] }} mb-2">
                        {{ $stat['value'] }}
                    </h3>
    ...
    <div class="container relative z-10">
        <div class="max-w-4xl mx-auto text-center text-white" x-data x-intersect="$el.classList.add('animate-fade-in')">
            ...
            <div class="flex flex-col sm:flex-row gap-4 justify-center">
                <a href="{{ route('register') }}" class="inline-flex items-center justify-center px-8 py-4 text-base font-semibold text-blue-600 bg-white rounded-xl hover:bg-gray-50 transition-all shadow-xl hover:shadow-2xl hover:-translate-y-0.5">
                    Daftar Sekarang - Gratis
                    <svg class="w-5 h-5 ml-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 7l5 5m0 0l-5 5m5-5H6"/>
                    </svg>
                </a>
                <a href="{{ route('about') }}" class="inline-flex items-center justify-center px-8 py-4 text-base font-semibold text-white bg-transparent border-2 border-white rounded-xl hover:bg-white/10 transition-all backdrop-blur-sm">
                    Pelajari Lebih Lanjut
                </a>
            </div>
            @else
            <a href="{{ auth()->user()->role === 'admin' ? route('admin.dashboard') : route('umkm.dashboard') }}" class="inline-flex items-center justify-center px-8 py-4 text-base font-semibold text-blue-600 bg-white rounded-xl hover:bg-gray-50 transition-all shadow-xl hover:shadow-2xl hover:-translate-y-0.5">
                Ke Dashboard
                <svg class="w-5 h-5 ml-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 7l5 5m0 0l-5 5m5-5H6"/>
                </svg>
