...
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        ...
        <h3 class="text-3xl font-bold text-gray-900 mt-2">{{ number_format($stats['total_umkm'], 0, ',', '.') }}</h3>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        ...
        <h3 class="text-3xl font-bold text-gray-900 mt-2">{{ number_format($stats['verified_umkm'], 0, ',', '.') }}</h3>
        <p class="text-xs text-gray-500 mt-1">
            {{ $stats['total_umkm'] > 0 ? number_format(($stats['verified_umkm'] / $stats['total_umkm']) * 100, 1) : 0 }}% dari total
        </p>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        ...
        <h3 class="text-3xl font-bold text-gray-900 mt-2">{{ number_format($stats['total_workers'], 0, ',', '.') }}</h3>
...
<div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
    <div class="p-6 border-b border-gray-200">
        <div class="flex items-center justify-between">
            ...
            <a href="{{ route('admin.umkm.index') }}" class="text-sm text-blue-600 hover:text-blue-700 font-medium">
                Lihat Semua →
            </a>
        </div>
    </div>
        ...
        <table class="w-full">
            ...
            <tbody class="divide-y divide-gray-200">
                ...
                <tr class="hover:bg-gray-50 transition-colors">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap">
                        <div class="text-sm text-gray-600">{{ $umkm->user->name }}</div>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap">
                        <div class="text-sm text-gray-600">{{ $umkm->kecamatan }}</div>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap">
                        @if($umkm->is_verified)
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <a href="{{ route('admin.umkm.show', $umkm) }}" class="text-blue-600 hover:text-blue-700">Detail</a>
                    </td>
                </tr>
                @empty
                <tr>
...
new Chart(kecamatanCtx, {
    type: 'bar',
    data: {
        labels: {!! json_encode($umkmByKecamatan->pluck('kecamatan')) !!},
        datasets: [{
            label: 'Jumlah UMKM',
            data: {!! json_encode($umkmByKecamatan->pluck('total')) !!},
...
const verifiedCount = {{ $stats['verified_umkm'] }};
const pendingCount = {{ $stats['total_umkm'] - $stats['verified_umkm'] }};
